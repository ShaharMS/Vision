...
class Test_vision_Vision_laplacianOfGaussianEdgeDetection
{
    public static function main()
    {
        ...
        vision.tools.ImageTools.loadFromFile("https://upload.wikimedia.org/wikipedia/commons/5/50/Vd-Orig.png", function(image)
        {
            for (i in 0...attempts)
            {
                ...
                vision.Vision.laplacianOfGaussianEdgeDetection(image);
                end = haxe.Timer.stamp();
                if (end - start > worst) worst = end - start;
                if (end - start < best) best = end - start;
                sum += end - start;
            }
            trace("----------vision.Vision.laplacianOfGaussianEdgeDetection()----------");
