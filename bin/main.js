...
vision_algorithms_Hough.toHoughSpaceWithRays = function(image,threshold,numLocalMaxima) {
	...
	var maximas = [];
	var rays = [];
	var _g = vision_ds_hough_HoughAccumulator.cellIterator(accum,50);
	while(_g.current < _g.array.length) {
		var point = _g.array[_g.current++];
		haxe_Log.trace(point == null ? "null" : vision_ds_IntPoint2D.toString(point),{ fileName : "src/vision/algorithms/Hough.hx", lineNumber : 100, className : "vision.algorithms.Hough", methodName : "toHoughSpaceWithRays"});
		var theta = vision_ds_IntPoint2D.get_y(point) * Math.PI / 180;
		var rho = vision_ds_IntPoint2D.get_x(point);
		var b = rho * (1 / Math.sin(theta));
		var tmp = Math.tan(theta);
		rays.push(new vision_ds_Ray2D(new vision_ds_Point2D(0,b),-(1 / tmp)));
	}
