...
class InvalidCramerCoefficientsMatrix extends VisionException {
	public function new(coefficients:Matrix2D) {
		super('Cramer coefficients matrix must be square (given matrix is ${coefficients.rows}x${coefficients.columns}). matrix:\n${coefficients}', "Invalid Cramer Coefficients Matrix");
	}
}